...
def display_instances(image, class_names, 
        boxes, masks, class_ids,
        pred_boxes, pred_masks, pred_class_ids, pred_scores=None, 
        title="", figsize=(13, 13), ax=None):
    """
    ...
    scores = np.ones(shape=class_ids.shape)
    ...
    for i in range(len(pred_boxes)):
        cov = 0
        for b in boxes:
            cov = max(cov, box_coverage(pred_boxes[i], b))
        if cov > 0.3:
            color = (1.0, 0.0, 0.0)
        else:
            color = (0.0, 1.0, 0.0)
        ...
        score = pred_scores[i] if scores is not None else None
        label = class_names[class_id]
        x = random.randint(x1, (x1 + x2) // 2)
        caption = "{} {:.1f}".format(label, score) if score else label
    ...
    plt.margins(0,0)
    plt.savefig('test.eps', format='eps', dpi=300, pad_inches = 0)
...
def display_boxed_image(image, boxes,
        title="", figsize=(13, 13), ax=None):
    """
    boxes: [num_instance, (y1, x1, y2, x2, class_id)] in image coordinates.
    masks: [height, width, num_instances]
